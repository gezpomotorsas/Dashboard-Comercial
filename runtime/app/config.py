...
def _resolve_env_file() -> str | None:
    ...
    if custom:
        ...
        if _env_file_is_configured(custom_path):
            return str(custom_path)

    data_dir = os.getenv("DASHBOARD_DATA_DIR")
    if data_dir:
        data_path = Path(data_dir)
        data_env = data_path / ".env"
        root_env = data_path.parent / ".env"
...
class Settings(BaseSettings):
    ...
    hubspot_timeout_seconds: float = Field(
        default=120.0,
        validation_alias=AliasChoices("HUBSPOT_TIMEOUT_SECONDS"),
    )
