...
export function flattenContactMethodology(
  ...
  return {
    ...
    'Llamadas — cobertura 7d (%)': pct(calls?.call_coverage_rate_7d),
    'Llamadas — cobertura 15d (%)': pct(calls?.call_coverage_rate_15d),
    'Llamadas — cobertura 30d (%)': pct(calls?.call_coverage_rate_30d),
    'Llamadas — cobertura ventana (%)': pct(calls?.call_coverage_rate),
...
export function advisorToExportRow(row: BrandAdvisorRow, staleDays: number): Record<string, ExportScalar> {
  ...
  return {
    ...
    'Cobertura llamadas 7d (%)': pct(row.call_coverage_rate_7d),
    'Cobertura llamadas 15d (%)': pct(row.call_coverage_rate_15d),
    'Cobertura llamadas 30d (%)': pct(row.call_coverage_rate_30d),
