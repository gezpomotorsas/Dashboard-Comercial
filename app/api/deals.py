...
router = APIRouter(prefix="/api/v1/hubspot/deals", tags=["hubspot-deals"])
...
@router.get("/{deal_id}", response_model=HubSpotObjectResponse)
async def get_deal(
    deal_id: str,
    properties: str | None = None,
    properties_with_history: str | None = None,
    ...
    return await deals_service.get_deal(
        client,
        deal_id=deal_id,
        properties=_split_csv(properties),
        properties_with_history=_split_csv(properties_with_history),
